  ...
  const fetchDashboard = async () => {
    ...
    try {
      ...
      if (isOffline) {
        ...
        // Ensure initialized
        if (typeof window !== "undefined" && !localStorage.getItem("offline_initialized")) {
          offlineStore.enableOfflineMode();
        }
        data = offlineStore.getDashboardStats(period);
      } else {
      ...
      try {
        const { offlineStore } = await import("@/lib/offlineStore");
        const data = offlineStore.getDashboardStats(period);
        setStats(data);
      } catch {}
    } finally {
      setLoading(false);
