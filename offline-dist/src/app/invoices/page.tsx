  ...
  const fetchInvoices = async () => {
    ...
    try {
      ...
      if (isOffline) {
        ...
        let allInvoices = offlineStore.getInvoices();
        if (search) {
        ...
        if (jobStatus) allInvoices = allInvoices.filter((inv) => inv.jobStatus === jobStatus);
        if (paymentStatus) allInvoices = allInvoices.filter((inv) => inv.paymentStatus === paymentStatus);
      ...
      try {
        const { offlineStore } = await import("@/lib/offlineStore");
        const allInvoices = offlineStore.getInvoices();
        setInvoices(allInvoices);
        setTotal(allInvoices.length);
      } catch {}
    } finally {
