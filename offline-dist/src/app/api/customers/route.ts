...
export async function GET(request: Request) {
  try {
    ...
    if (isOffline || !db) {
      const allCustomers = serverOfflineStore.getCustomers(search || undefined);
    ...
    try {
      ...
      const allCustomers = serverOfflineStore.getCustomers(search);
      return NextResponse.json({ customers: allCustomers });
    } catch {
...
export async function POST(request: Request) {
  try {
    ...
    if (isOffline || !db) {
      const customer = serverOfflineStore.createCustomer(validated);
